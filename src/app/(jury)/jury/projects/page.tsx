...
import { FolderOpen, ArrowRight, FileText, Trophy } from "lucide-react"
...
export default async function JuryProjectsPage() {
  ...
  const projects = await prisma.project.findMany({
    ...
    include: {
      ...
      reports: { where: { juryId: null }, select: { id: true } },
    },
    ...
    <div className="p-4 sm:p-8 space-y-6">
      <div className="pb-5 border-b border-black/[0.06]">
        ...
        <p className="text-[13px] text-[#6e6e73] mt-2">
          {projects.length === 0
            ? "Değerlendirdiğiniz projelerden henüz desteklenen yok."
      ...
      {projects.length === 0 ? (
        <div className="bg-white rounded-2xl border border-black/[0.06] shadow-[0_1px_6px_rgba(0,0,0,0.04)] p-14 text-center">
          <div className="w-12 h-12 rounded-2xl bg-[#f5f5f5] flex items-center justify-center mx-auto mb-4">
            <Trophy className="w-6 h-6 text-[#aeaeb2]" />
          </div>
          <p className="text-[15px] font-semibold text-[#1c1c1c]">Henüz desteklenen proje yok</p>
          <p className="text-[13px] text-[#6e6e73] mt-1.5">
            Değerlendirdiğiniz başvurular desteklendiğinde burada görünecek.
          </p>
        </div>
      ) : (
        <div className="space-y-2.5">
          {projects.map((p) => {
            ...
            return (
              <Link
                key={p.id}
                href={`/jury/projects/${p.application.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      ...
                      <span className="inline-flex items-center gap-1 text-[11px] text-[#aeaeb2]">
                        <FileText className="w-3 h-3" /> {p.reports.length} güncelleme
                      </span>
                    </div>
                  </div>
