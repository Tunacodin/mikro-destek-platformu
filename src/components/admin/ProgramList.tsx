...
const NEXT_STATUS: Partial<Record<ProgramStatus, { label: string; status: ProgramStatus }>> = {
...
export function ProgramList({ programs }: { programs: Program[] }) {
  ...
  async function changeStatus(id: string, status: ProgramStatus) {
    if (pendingRef.current) return
    pendingRef.current = true
    setLoading(id)
    try {
      await fetch(`/api/admin/programs/${id}/status`, {
        method: "PATCH",
